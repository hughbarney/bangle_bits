...
const writeOutput = function() {
  ...
  //g.setFont12x20(2);
  g.setFont('Vector',30);
  g.drawString(processedData.temperature.toFixed(1) + ' °C', 10, 30);
  //g.drawString(`${processedData.humidity.toFixed(1)} %`, 10, 70);

  //g.setFont6x15();
  //g.setFont('6x8',2);
  g.setFont('Vector',20);
  g.drawString(`${((now - lastSeen[macs[current]]) / 60000).toFixed(0)} min ago`, 10, 100);
  g.drawString(`${processedData.battery_level} % battery`, 10, 120);
  g.drawString(`${processedData.MAC in aliases ? aliases[processedData.MAC] : processedData.MAC}`, 10, 140);
  g.drawString(`${current + 1} / ${macs.length}`, 10, 150);
};
