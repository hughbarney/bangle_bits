...
var buf = Graphics.createArrayBuffer(100,100,2,{msb:true});
var candraw = true;

function flip(b,x,y) {
 g.drawImage({width:100,height:100,bpp:2,buffer:b.buffer, palette:pal2color},x,y);
 b.clear();
}
...
function drawCompass(course) {
  ...
  buf.setColor(1);
  buf.drawImage(img, 50, 50, {scale:1.5, rotate: radians(course)} );  
  flip(buf, 70, 30);
}
...
function drawN(){
  ...
  buf.setColor(3);
  buf.drawString("Brg: ",0,40);
  buf.drawString("Dist: ",0,70);
  buf.setColor(1);
  buf.drawString(wp_bearing,60,40);
  buf.drawString(loc.distance(dist),60,70);
  flip(buf, 0, 130);

  // waypoint name
  buf.setColor(selected?1:2);
  buf.drawString(wp.name,20,40);
  flip(buf, 120, 130);
}
