 ...
 * v3.6 changed gating mechanism so must exceeed V_RAW_THRESHOLD raw for 2 samples out of every 13
 *
 */
...
const V_RAW_THRESHOLD = 45;   // raw threshold we must cross to start registering raw acceleration values
...
function onAccel(a) {
   ...
   *
   * To screen out movement like driving we must have at least 3
   * samples over threshold in any one period of 13 samples.
   *
   * We ignore the negative samples as the average is generally positive.
   *
   * This reduces the impact of the ringing on the filter to within
   * +/- 1 second almost a perfect filter. It means we might over
   * count by 1 step due to late shutting off BUT this is balanced by
   * late switching on when we first detected acceleration.
   *
   */

  if (raw_clipped > V_RAW_THRESHOLD) {
    if (high_samples < 2)
      high_samples++;
    if (high_samples == 2)
      low_samples = 0; // we must count up to 10 lows to close the gate
  } else {
    if (low_samples < 11)
      low_samples++;
    if (low_samples == 11)
      high_samples = 0; // we must count some more to reopen the gate
  }
  ...
  console.log(t + "," + raw_clipped + "," + accFiltered +"," + high_samples);
