...
unsigned char lastMinRejectedSteps; // previous hold steps in last 60 seconds
unsigned short int stepLength; // how many poll intervals since the last step?
...
int stepcount_had_step() {
  ...
  case S_STEP_1:
    ...
    if (stepLength <= T_MAX_STEP && stepLength >= T_MIN_STEP) {
      ...
      // reset lastMinRejectedSteps if we have not had a step attempt in last 60 seconds
      if (stepLength > 750)
        lastMinRejectedSteps = 0;
    }
    return 0;
  ...
  case S_STEP_22N:
	...
	return X_STEPS + lastMinRejectedSteps;
      }
      ...
      if (stepLength > 750)
        ...
        lastMinRejectedSteps = holdSteps;
    }
    return 0;
